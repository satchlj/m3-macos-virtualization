...
from .events import allocation, exceptions, handoff, native_platform, retype, txm_step, txm_trace
    ...
    try:
        ...
        if run.handoff_state.get('native'):
            ...
            if run.dockchannel_mmio is not None and run.report.get('handoff', {}).get('target_pc'):
                event_state.native_dockchannel_site = run.match_xnu_dockchannel_uart(
                    event_state.native_ctx, int(run.report['handoff']['target_pc'], 0), run.dockchannel_mmio)
            if run.panic_carveout is not None and run.report.get('handoff', {}).get('target_pc'):
                event_state.native_panic_carveout = run.match_xnu_panic_carveout(
                    event_state.native_ctx, int(run.report['handoff']['target_pc'], 0))
            if run.socd_trace is not None and run.report.get('handoff', {}).get('target_pc'):
                event_state.native_socd_trace = run.match_xnu_socd_trace(
                    event_state.native_ctx, int(run.report['handoff']['target_pc'], 0))
            ...
            if (event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC
                    and int(event_state.native_ctx.esr) >> 26 == 0x16):
                ...
                event_state.expected = run.struct.pack('<I', 0xd4000002 | ((int(event_state.native_ctx.esr) & 0xffff) << 5))
                ...
                if run.a.xnu_pperm_guest_window:
                    event_state.tag = int(event_state.native_ctx.esr) & 0xffff
                    for event_state.index, event_state.site in enumerate(run.FC_XNU_PPERM_SITES):
                        ...
                        if (event_state.tag == event_state.expected_tag and event_state.native_ctx.elr - 4 == event_state.runtime_pc
                                and event_state.source_original == run.struct.pack('<I', event_state.word)):
                            event_state.native_xnu_pperm_site = (event_state.index, event_state.site)
                            event_state.native_sptm_callback = False
                            break
            if (event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC
                    and int(event_state.native_ctx.esr) == run.FC_XNU_AGT_ESR):
                event_state.segment = run.layout.get('images', {}).get('kernelcache', {}).get('segments', {}).get('__TEXT_EXEC', {})
                event_state.within_segment = (event_state.segment.get('va', 0) <= run.FC_XNU_AGT_LINKED
                                  and run.FC_XNU_AGT_LINKED + 4 <= event_state.segment.get('va', 0) + event_state.segment.get('filesize', 0))
                event_state.source_off = event_state.segment.get('fileoff', 0) + run.FC_XNU_AGT_LINKED - event_state.segment.get('va', 0)
                event_state.original_word = (run.sources.get('kernelcache', b'')[event_state.source_off:event_state.source_off+4]
                                 if event_state.within_segment else b'')
                event_state.runtime_pc = (int(run.report.get('handoff', {}).get('target_pc', '0'), 0)
                              + run.FC_XNU_AGT_LINKED - run.FC_XNU_ENTRY_LINKED)
                event_state.native_xnu_agt_callback = (event_state.native_ctx.elr - 4 == event_state.runtime_pc
                    and event_state.original_word == run.struct.pack('<I', run.FC_XNU_AGT_WORD))
            if (event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC
                    and int(event_state.native_ctx.esr) == run.FC_XNU_CNTP_CTL_ESR):
                event_state.segment = run.layout.get('images', {}).get('kernelcache', {}).get('segments', {}).get('__TEXT_EXEC', {})
                event_state.within_segment = (event_state.segment.get('va', 0) <= run.FC_XNU_CNTP_CTL_LINKED
                                  and run.FC_XNU_CNTP_CTL_LINKED + 4 <= event_state.segment.get('va', 0) + event_state.segment.get('filesize', 0))
                event_state.source_off = event_state.segment.get('fileoff', 0) + run.FC_XNU_CNTP_CTL_LINKED - event_state.segment.get('va', 0)
                event_state.original_word = (run.sources.get('kernelcache', b'')[event_state.source_off:event_state.source_off+4]
                                 if event_state.within_segment else b'')
                event_state.runtime_pc = (int(run.report.get('handoff', {}).get('target_pc', '0'), 0)
                              + run.FC_XNU_CNTP_CTL_LINKED - run.FC_XNU_ENTRY_LINKED)
                event_state.native_xnu_cntp_ctl_callback = (event_state.native_ctx.elr == event_state.runtime_pc
                    and event_state.original_word == run.struct.pack('<I', run.FC_XNU_CNTP_CTL_WORD))
            if (run.a.xnu_apple_physical_timer_hypothesis
                    and event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC
                    and int(event_state.native_ctx.esr) == run.FC_XNU_APPLE_PHYS_TIMER_ESR):
                event_state.segment = run.layout.get('images', {}).get('kernelcache', {}).get('segments', {}).get('__TEXT_EXEC', {})
                event_state.within_segment = (event_state.segment.get('va', 0) <= run.FC_XNU_APPLE_PHYS_TIMER_LINKED
                                  and run.FC_XNU_APPLE_PHYS_TIMER_LINKED + 4 <= event_state.segment.get('va', 0) + event_state.segment.get('filesize', 0))
                event_state.source_off = event_state.segment.get('fileoff', 0) + run.FC_XNU_APPLE_PHYS_TIMER_LINKED - event_state.segment.get('va', 0)
                event_state.original_word = (run.sources.get('kernelcache', b'')[event_state.source_off:event_state.source_off+4]
                                 if event_state.within_segment else b'')
                event_state.runtime_pc = (int(run.report.get('handoff', {}).get('target_pc', '0'), 0)
                              + run.FC_XNU_APPLE_PHYS_TIMER_LINKED - run.FC_XNU_ENTRY_LINKED)
                event_state.native_xnu_apple_timer_callback = (event_state.native_ctx.elr == event_state.runtime_pc
                    and event_state.original_word == run.struct.pack('<I', run.FC_XNU_APPLE_PHYS_TIMER_WORD))
        ...
        if run.watchdog is not None and run.watchdog.expired:
        ...
        elif ((run.phase53_allocation_trace_state.get('active') or
               run.phase53_retype_survey_state.get('active')) and
              run.handoff_state.get('native') and
              event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC and
              int(event_state.native_ctx.esr) >> 26 == 0x32 and
              event_state.native_ctx.elr == run.FC_XNU_TXM_HANDLER_CMD1_RETAB):
            allocation.handle_authenticated_return(run, event_state)
        elif (run.phase53_retype_survey_state.get('active') and
              run.phase53_retype_survey_state.get('stage') == 'seek-entry' and
              run.phase53_retype_survey_state.get('completed_calls', 0) > 0 and
              run.phase53_retype_survey_state.get('range0') ==
                  run.FC_TXM_RUNTIME_TEXT and
              run.handoff_state.get('native') and
              event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC and
              int(event_state.native_ctx.esr) >> 26 == 0x32 and
              run.FC_XNU_RUNTIME_TEXT[0] <= event_state.native_ctx.elr <
            ...
            retype.handle_unrelated_return(run, event_state)
              ...
              event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC and
              int(event_state.native_ctx.esr) >> 26 == 0x32 and
              event_state.native_ctx.elr == run.FC_XNU_PHASE53_GENTER_RETURN and
              run.phase53_retype_survey_state.get('stage') ==
                  'seek-genter-return'):
            retype.handle_guarded_return(run, event_state)
        elif (run.phase53_allocation_trace_state.get('active') and
              run.handoff_state.get('native') and
              event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC and
              int(event_state.native_ctx.esr) >> 26 == 0x32 and
              event_state.native_ctx.elr == run.FC_XNU_PHASE53_GENTER_RETURN):
            allocation.handle_guarded_return(run, event_state)
        elif (run.phase53_retype_survey_state.get('active') and
              event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC and
              int(event_state.native_ctx.esr if run.handoff_state.get('native') else
                  run.iface.readstruct(event_state.info, run.ExcInfo).esr) >> 26 == 0x32):
        ...
        elif (run.txm_validator_trace_state.get('active') and
              run.txm_validator_trace_state.get('phase') == 'completion' and
              run.txm_validator_trace_state.get('fast_path_armed') and
              run.handoff_state.get('native') and
              event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC and
              int(event_state.native_ctx.esr) == run.FC_XNU_TXM_HANDLER_CMD1_MDSCR_ESR and
              event_state.native_ctx.elr == run.FC_XNU_TXM_HANDLER_CMD1_MDSCR_PC):
            txm_trace.handle_debug_gate(run, event_state)
        elif (run.txm_validator_trace_state['active'] and
              not event_state.native_sptm_callback and
              (not run.handoff_state.get('native') or
        ...
        elif (run.handoff_state.get('native') and event_state.native_dockchannel_site is not None
              and event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC):
            native_platform.handle_console(run, event_state)
        elif (run.handoff_state.get('native') and event_state.native_panic_carveout
              and event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC):
            native_platform.handle_panic_buffer(run, event_state)
        elif (run.handoff_state.get('native') and event_state.native_socd_trace
              and event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC):
            native_platform.handle_trace_buffer(run, event_state)
              ...
              and event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC
              and int(event_state.native_ctx.esr) >> 26 == 0x18):
            native_platform.handle_debug_access(run, event_state)
        elif run.handoff_state.get('native') and not event_state.native_sptm_callback:
            event_state.ctx = event_state.native_ctx
            event_state.event.update(kind='xnu-native-exception', pc=event_state.ctx.elr, esr=int(event_state.ctx.esr),
        ...
        if event_state.ret == run.EXC_RET.HANDLED and 'sptm_panic' not in run.report:
            ...
            for event_state._pi, event_state._ev in enumerate(run.report.get('trace', [])):
                if event_state._ev.get('pc') == run.FC_PANIC and isinstance(event_state._ev.get('regs'), list) and len(event_state._ev['regs']) >= 31:
                    ...
                    run.report['sptm_panic'] = dict(pc=run.FC_PANIC, args=list(event_state._r[:6]), lr=event_state._r[30],
                        prior_pcs=[e.get('pc') for e in run.report['trace'][max(0,event_state._pi-16):event_state._pi]])
        ...
        if event_state.ret == run.EXC_RET.HANDLED and run.guarded_vector_stop is not None:
            ...
            try:
                if not run.guarded_vbar:
                    ...
                    if event_state.last and isinstance(event_state.last.get('spsr'), int) and (event_state.last['spsr'] & 0x400000):
        ...
        if event_state.ret == run.EXC_RET.HANDLED and run.a.first_contact:
            ...
            event_state.reason = {run.FC_STOP: 'first-contact-service-boundary',
                      run.FC_INIT: 'first-contact-wrong-entry-init',
                      run.FC_SERVICE: 'first-contact-service-entered'}.get(event_state.pc)
            if event_state.reason is not None:
                event_state.ret = run.EXC_RET.EXIT_GUEST
                run.report['stop_reason'] = event_state.reason
                event_state.fc = dict(stop_pc=hex(event_state.pc), reason=event_state.reason,
                          reached_dispatcher=any(e.get('pc') == run.FC_DISPATCH for e in run.report.get('trace', [])))
                try:
        ...
        if event_state.ret == run.EXC_RET.HANDLED and run.a.real_guarded and 'sptm_panic' not in run.report:
            ...
            if event_state.last and event_state.last.get('pc') == run.FC_PANIC:
                ...
                run.report['sptm_panic'] = dict(pc=hex(run.FC_PANIC), lr=_h(30),
        ...
        try:
            ...
            if (run.a.free_run and not run.handoff_state['active']
                    and not run.txm_context_step_state['active']
                    and not run.txm_validator_trace_state['active']
                    and event_state.ret == run.EXC_RET.HANDLED and run.entered):
            ...
            if run.batch is not None:
                try:
                    if (not run.a.free_run and event_state.ret == run.EXC_RET.HANDLED and run.entered and run.shadow_sctlr & 1
                            and not (run.a.single_step_after and run.trace_count(run.report) >= run.a.single_step_after)
                            and not (run.ss_window and run.ss_window[0] <= run.trace_count(run.report) < run.ss_window[1])):
