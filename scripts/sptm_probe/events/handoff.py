...
def handle_bounded_handoff(run, event_state):
    ...
    event_state.event.update(kind='handoff-step', pc=event_state.ctx.elr, esr=int(event_state.ctx.esr), spsr=int(event_state.ctx.spsr), regs=list(event_state.ctx.regs), far=event_state.ctx.far, sp=list(event_state.ctx.sp))
    ...
    if event_state.reason == run.START.EXCEPTION_LOWER and event_state.code == run.EXC.SYNC and (int(event_state.ctx.esr) >> 26 == 50):
        ...
        if run.handoff_state['events'] < run.handoff_state.get('budget', run.a.handoff_steps):
        ...
        elif run.handoff_state.get('xnu') and run.a.xnu_run and run.report['handoff'].get('instructions_executed'):
            event_state.roots = dict(ttbr0=run.u.mrs(run.TTBR0_EL12), ttbr1=run.u.mrs(run.TTBR1_EL12))
            event_state.continuation = run.classify_entry(event_state.ctx.elr, run.layout, run.base, run.guest_size, event_state.roots, run.iface.readmem, run.sources)
            if event_state.continuation['image'] == 'kernelcache' and event_state.continuation['bytes_match']:
                ...
                if run.a.xnu_tpidr_gl2_fast_shadow:
                    ...
                    try:
                        event_state.enabled_status = run.tpidr_gl2_fast_shadow.enable(run.tpidr_gl2_shadow_tag_base, event_state.initial_value)
                        run.report['xnu_tpidr_gl2_fast_shadow'].update(activated=True, initial_value=event_state.initial_value, initial_value_hex=hex(event_state.initial_value), enable_status=event_state.enabled_status, activated_at_trace_index=run.trace_count(run.report), activated_from_pc=hex(event_state.ctx.elr))
...
def handle_guarded_exit(run, event_state):
    ...
    if event_state.elr_gl1:
        ...
        event_state.phase53_eret_candidate = bool(run.phase53_allocation_trace_state.get('active') or run.phase53_retype_survey_state.get('active'))
        event_state.phase53_eret = False
        if event_state.phase53_eret_candidate:
            event_state.phase53_filter_state = run.phase53_retype_survey_state if run.phase53_retype_survey_state.get('active') else run.phase53_allocation_trace_state
            event_state.phase53_report_key = 'xnu_phase53_retype_survey' if run.phase53_retype_survey_state.get('active') else 'xnu_phase53_allocation_trace'
            ...
            try:
                ...
                event_state.transition_checks.update(filter_running=event_state.prior_status['active'] and event_state.prior_status['status'] == run.FC_VEL2_STEP_FILTER_RUNNING, filter_bounded=0 < event_state.prior_status['steps'] <= run.FC_XNU_PHASE53_FAST_STEPS, filter_last_pc=event_state.prior_status['last_pc'] == event_state.ctx.elr - 4, filter_segment=event_state.prior_status['first_pc'] == event_state.segment_start and event_state.prior_status['expected_first_pc'] == event_state.segment_start and (event_state.prior_status['range0_hits'] + event_state.prior_status['range1_hits'] == event_state.prior_status['steps']), filter_contract=event_state.prior_status['range0_start'] == event_state.current_range[0] and event_state.prior_status['range0_end'] == event_state.current_range[1] and (event_state.prior_status['range1_start'] == run.FC_SPTM_RUNTIME_TEXT[0]) and (event_state.prior_status['range1_end'] == run.FC_SPTM_RUNTIME_TEXT[1]) and (event_state.prior_status['terminal_pc'] == event_state.phase53_filter_state['terminal_pc']) and (event_state.prior_status['max_steps'] == run.FC_XNU_PHASE53_FAST_STEPS))
                if run.phase53_retype_survey_state.get('active'):
                    event_state.transition_checks.update(aggregate_budget=event_state.survey_aggregate <= run.FC_XNU_PHASE53_RETYPE_SURVEY_TOTAL_STEPS, rearm_budget=event_state.phase53_filter_state['rearms'] < run.FC_XNU_PHASE53_RETYPE_SURVEY_MAX_REARMS)
                if all(event_state.transition_checks.values()):
                    event_state.enabled_status = run.txm_sstep_fast_path.enable(event_state.target_range, run.FC_SPTM_RUNTIME_TEXT, event_state.phase53_filter_state['terminal_pc'], run.FC_XNU_PHASE53_FAST_STEPS, event_state.elr_gl1)
